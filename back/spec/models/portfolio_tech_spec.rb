  ...
  describe 'バリデーション' do
    ...
    describe '必須項目' do
      it 'portfolio が空の場合エラーになること' do
        ...
        expect(portfolio_tech.errors[:portfolio]).to eq ['must exist']
      end
      ...
      it 'tech が空の場合エラーになること' do
        ...
        expect(portfolio_tech.valid?).to be false
        expect(portfolio_tech.errors.size).to eq 1
        expect(portfolio_tech.errors[:tech]).to eq ['must exist']
      end
    end
    ...
    describe '重複' do
      it 'portfolioとtech の組み合わせが重複する場合エラーになること' do
        ...
        expect(another_portfolio_tech.errors[:portfolio_id]).to eq ['has already been taken']
      end
    end
